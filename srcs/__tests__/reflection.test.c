...
static t_object_wrapper			get_sphere(void)
{
	...
	args_sphere.r = 1.0f;
	args_sphere.c = (t_vec4){{0.0f, 10.0f, 0.0f, 1}};
	return (new_sphere(&args_obj, &args_sphere));
}

t_object_wrapper				get_mirror(t_vec4 n, t_vec4 p)
{
	t_new_plane_args			args_plane;
	t_new_object_args			args_obj;

	args_obj.reflectivity = 1.0f;
	args_obj.transparency = 0.0f;
...
void							test_reflection(void)
{
	...
	object_wrappers[1] = get_mirror(
		(t_vec4){{-1.0f, -1.0f, 0.0f, 1.0f}},
		(t_vec4){{30.0f, 0.0f, 0.0f, 1.0f}});
	object_wrappers[2] = get_mirror(
		(t_vec4){{1.0f, 1.0f, 0.0f, 1.0f}},
		(t_vec4){{-30.0f, 0.0f, 0.0f, 1.0f}});
